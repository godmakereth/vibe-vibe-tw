---
title: "7.2.5 串起整個請求——追蹤 ID：請求鏈路跟蹤與調試"
...
當用戶說"出錯了"，你需要 Trace ID 來找到那一條請求的完整日誌——這是定位問題的鑰匙。

## 爲什麼需要 Trace ID

```mermaid
flowchart LR
    Client["客戶端"] --> API["API 網關"]
    API --> Service1["服務 A"]
...
| 問題 | 沒有 Trace ID | 有 Trace ID |
|------|--------------|-------------|
| 用戶報錯 | 哪個請求出錯了？ | 根據 ID 查日誌 |
| 定位問題 | 在海量日誌中搜索 | 精確過濾 |
| 性能分析 | 無法串聯請求鏈路 | 完整鏈路追蹤 |
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: '創建失敗', traceId } },
      { status: 500 }
    )
  }
}
...
async function callExternalService(data: unknown, traceId: string) {
  const response = await fetch('https://external-api.com/endpoint', {
    ...
    headers: {
      ...
      'X-Trace-ID': traceId,  // 傳遞 Trace ID
    },
...
// ✅ 錯誤響應包含 Trace ID
return NextResponse.json(
  { error: { code: 'ERROR', message: '失敗', traceId } },
  { status: 500 }
)
```
